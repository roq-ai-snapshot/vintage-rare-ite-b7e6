...
import { useOrderCount, useOrderItemCount, useItemCount } from 'lib/roq';

import { InventoryListPage as ShowCaseList } from 'pages/inventories';

function DashboardPage() {
  const { session } = useSession();

  const orderStat = useOrderCount();

  const orderItemStat = useOrderItemCount();

  const itemStat = useItemCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Orders',
      result: orderStat?.data,
      isLoading: orderStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Order Items',
      result: orderItemStat?.data,
      isLoading: orderItemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Items',
      result: itemStat?.data,
      isLoading: itemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`inventories`} passHref legacyBehavior>
                  <Button
